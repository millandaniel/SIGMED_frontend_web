...
function FormEps() {
    ...
    const agregarEps = async (e) => {

        if (!nombre.trim()) {
            alert("El campo nombre no puede ser vacio*")
            return;
        } else {
            const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/eps/agregar/`, {
                method: "POST",
                body: JSON.stringify({
                    nombre: nombre
                }),
    ...
    return (
        <div>
            <div className=" bg-gray-50 flex justify-between items-center">
                <h2 className=" text-center ml-4 text-5xl">Lista de Eps</h2>
                <button className="font-semibold text-white rounded-md p-2 m-2 mr-4" onClick={() => handleOpenModal()} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Agregar Una Eps</button>
            </div>
            <br />
                ...
                <div className="modal-dialog">
                    <div className="modal-content align-middle justify-center">
                        <div className="modal-header">
                            <h5 className="modal-title text-2xl font-bold align-middle justify-center" id="exampleModalLabel">Agregar Una Eps</h5>
                        ...
                        <div className="modal-footer" >
                            <button className="bg-green-500 text-white rounded-md p-2" onClick={() => agregarEps()}>Agregar Eps</button>
