...
function EspecialidadCard({ especialidades }) {
    ...
    const cargarDatosEspecialidad = (especialidad) => {
        setnuevonombre(especialidad.nombre);
    };

    //Funcion para eliminar una especialidad de la base de datos
    const eliminarEspecialidad = async (id) => {
        console.log(id)
        if (window.confirm("¿Desea eliminar esta Especialidad?")) {
            const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/especialidades/eliminar/`, {
                method: "DELETE",
                headers: {
    ...
    const modificarEspecialidad = async (id) => {
        console.log("Modificacion de especialidad: ", "Id: ", id, "Nombre: ", nuevonombre)
        if (!nuevonombre.trim()) {
            alert("No puede asignarle un nombre vacio a la Especialidad")
            return;
        } else {
            if (window.confirm("¿Deseas modificar esta especialidad?")) {
                //-------Mirar la verificacion de los campos si es necesaria
                const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/especialidades/modificar/`, {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json"
                    },
    ...
    const handleOpenModal = (row) => {
        setSelectedRow(row);
        cargarDatosEspecialidad(row); // Cargar los datos de la especialidad seleccionada
    ...
    return (
        <div className="rounded-md justify-between">
                ...
                <DataTable
                    ...
                    columns={[
                        ...
                        { // Columna personalizada para el botón "Eliminar"
                            cell: (row) => <button className="font-semibold text-white rounded-md p-2 m-2 bg-red-600" onClick={() => eliminarEspecialidad(row.id)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Eliminar</button>,
                            allowOverflow: true,
                            button: true,
                            width: "100px"
                        }
            ...
            {/* Modal para mostrar la modificacion de la especialidad seleccionada */}

            <div className="modal fade" id="modal" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div className="modal-dialog">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title" id="exampleModalLabel">Modificar Especialidad</h5>
                        ...
                        <div className="modal-footer" >
                            <button className="bg-green-500 text-white rounded-md p-2" onClick={() => modificarEspecialidad(selectedRow.id)}>Guardar Cambios</button>
